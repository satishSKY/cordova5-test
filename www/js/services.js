      ...
      xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
          if (xhr.responseXML.documentElement != null || xhr.responseXML.documentElement != 'null') {
            ...
            if ($rootScope.deviceWidth < $rootScope.deviceHeight)
              $('svg').height($rootScope.deviceWidth);
            else
              $('svg').height($rootScope.deviceWidth / 2);

            $('svg').width($rootScope.deviceWidth);
             var path = document.getElementsByTagName("path");
             for (var int = 0; path.length > int; int++) {
              path[int].onclick = function (e) {
